...
\begin{equation}\label{eq: path_nonfield}
  U(\mathbf{x_a},\mathbf{x_b};T)=\langle \mathbf{x_b}|e^{-i\hat{H}T/\hbar}|\mathbf{x_a} \rangle = \sum_{all\ paths} e^{iS[\mathbf{x}(t)]/\hbar} = \int \mathcal{D}\mathbf{x}(t)e^{iS[\mathbf{x}(t)]/\hbar},
\end{equation}
...
令$\phi^*(k) = \phi(-k)$可以保证$\text{Im}\ \phi(-k_n) = -\text{Im}\ \phi(k_n)$, 这样在求和的时候虚部就会全部抵消.
