  ...
  \item 关联函数、传播子以及费曼规则
        \begin{enumerate}
          \item 在路径积分formalism中, 我们通过书中(9.14)式从右向左由Lagrangian dynamics(以路径积分原理为量子化的基础)定义了Hamiltonian dynamics(以算符的对易关系为量子化的基础).
                具体来说, 是\ {\color{Maroon}{$\langle \phi_b(\mathbf{x})|e^{-i\hat{H}t}|\phi_a(\mathbf{x})\rangle$}} (即transition function, 含有\cemph{哈密顿算符}以及\cemph{其本征态张成的希尔伯特空间}的信息)被\ {\color{Maroon}$\exp{\Bigl[i\int_0^T d^4 x \mathcal{L}}\Bigr]$} (某种相位, 与\cemph{理论相应的作用量}有关)的{\color{Maroon}某种和}(路径积分, 在位形空间中一点到另一点的所有路径上求和)定义.
