...
\begin{itemize}
  \item 高斯积分
        \mybox{
          (\textit{个人理解})在$\int d^4x\ \overline{\psi}(i\cancel{\partial}-m)\psi$中, $\psi$和$\overline{\psi}$是两个向量, $(i\cancel{\partial}-m)$是线性算子, $\int d^4x$是定义在此空间上的内积. 所以这个积分就是两个向量的内积, 可以用前面的高斯积分来计算.
        }
